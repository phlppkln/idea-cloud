...
export default function Main( {boards}: {boards: string[]}) {
  ...
  const startNodeExplorer = async () => {
    console.log('panel.tsx init()');
    ...
    <div className="welcome-screen">
      ...
      <div>
        <ol>
          ...
          <li>
            Next, ask players to silently visit each node in the grid and to add sticky
            notes with exactly one word in response to the question for each node. 
            <strong>
              Important: Players need to put the sticky notes in the frame
              surrounding the corresponding image. Otherwise the sticky notes can not be
              assigned to a node.
            </strong>
            Depending on your question, this should take about half a minute to a minute per node. 
            You can also instruct players to create more sticky notes per node, but this will increase the time needed to play the game.
            We suggest that every player should leave at least three sticky notes at each node.
            Furthermore, the notes are not allowed to have a white background color.
          </li>
          <li>
            Afterwards you can start the clustering part of the game. To do so, select the nodes that consist of frames that feature the image and the players notes. 
            With the game space selected press the "Start clustering" button. This will generate copies of the images next to the nodes. 
          </li>     
          <li>
            Now ask players to help you organize the images into clusters of images based on their relationship. Involve the group in this process as much as possible. 
          </li>

            Each image is surrounded by a rectangle that is used to convey the underlying similarity of the notes.  These Involve the players in this process as much as possible. 
          <li>Don't let the players spend and inordinate amount of time to cluster the images. </li>
        </ol>
      </div>
      <h2>Strategy</h2>
